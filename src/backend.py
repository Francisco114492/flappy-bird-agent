...
class Pipe:
    ...
    def collisions(self, players: list, world_height: int) -> set:
        """
        ...
        Returns:
            set: players that collided with the pipe
        """
        keys_to_remove = set()

        rect_top = Rect(int(self.px), 0, self.WIDTH, self.py_top)
        rect_bottom = Rect(
            int(self.px), self.py_bottom, self.WIDTH, world_height - self.py_bottom
        )
        ...
        for k in players:
            ...
            if rect_rect_collision(rect_player, rect_top) or rect_rect_collision(
                rect_player, rect_bottom
            ):
...
class World:
        ...
        if self.with_pipes:
            ...
            if self.pipes:
                ...
                keys_to_remove.update(
                    closest_pipe.collisions(self.players, self.HEIGHT)
                )
    ...
    def dump(self) -> dict:
        ...
        rv = {
            ...
            "players": players,
            "pipes": [
                {"px": p.px, "py_t": p.py_top, "py_b": p.py_bottom} for p in self.pipes
            ],
        }
        ...
        while True:
            ...
            while not done:
                ...
                for k in keys_to_remove:
                    await k.send(
                        json.dumps({"evt": "done", "highscore": self.world.highscore})
                    )
                ...
                if self.world.number_players() == 0:
                    ...
                    for p in players_to_remove:
                        await p.send(
                            json.dumps(
                                {"evt": "done", "highscore": self.world.highscore}
                            )
                        )
...
async def main(args: argparse.Namespace) -> None:
    ...
    async with websockets.serve(game.incomming_handler, "localhost", args.p):
        await game.mainloop(args)
